...
function game_core(isServer){ // takes truthy or falsey value
	...
	this.newInput = function(playerID, input){
		...
		for(var i=0; i<player.input.length; i++){
			if(player.input[i].inputID > input[input.length-1].inputID){
				// found some newer values from player, copy remaining player.input to input
				while(i < player.input.length){
	...
	*/
	...
	this.processInput = function(player){ // process all input in list.
		...
		if(this.isServer){
			...
			player.input[player.input.length-1].move.x = move.x;
			player.input[player.input.length-1].move.y = move.y;
	...
	this.getPlayerFistForce = function(player){
		return ( this.PlayerFistForce + Math.abs( getDistance(0,0, player.input[0].move.x, player.input[0].move.y ) ) ) * (1.0 + player.stats.FIST_FORCE);
	}
